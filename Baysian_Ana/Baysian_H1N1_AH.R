...
influenza_type <- "H1_A"
DicreteSIDistr <- Dicret_SI(mu = 3.26,sigma=1.93,day = 10)
...
analysis.tem.3 <- mutate(analysis.tem.3,idd = paste(region,winter,sep="-1"))
#analysis.tem.3 <- mutate(analysis.tem.3,SH = 1000*SH(WVP2(RH,SVP(C2K(MT)))))
...
H1N1_AH_Rain_Region_ID_Work <- brm(bf(log(adjust.rc)~s(AH)+s(rain)+(1|region)+(1|idd)+work),
                                   data = analysis.tem.3,
                                   prior = prior,
                                   chains = 6,cores = 6,control = list(adapt_delta = 0.99),iter = 2000)
#save.image("mtrhrainsunshine_idd.RData")
save(H1N1_AH_Rain_Region_ID_Work,file = paste(influenza_type,"AH_Rain_Region_ID_Work_Region.RData",sep = "_"))
...
H1N1_AH_Rain_Sun_Region_ID_Work <- brm(bf(log(adjust.rc)~s(AH)+s(rain)+s(sunshine)+(1|region)+(1|idd)+work),
                                       data = analysis.tem.3,
                                       prior = prior,
                                       chains = 6,cores = 6,control = list(adapt_delta = 0.99),iter = 2000)
#save.image("mtrhrainsunshine_idd.RData")
save(H1N1_AH_Rain_Sun_Region_ID_Work,file = paste(influenza_type,"AH_Rain_Sun_Region_ID_Work_Region.RData",sep = "_"))