...
influenza_type <- "B_A"
DicreteSIDistr <- Dicret_SI(mu = 3.72,sigma=1.95,day = 10)
...
B_AH_Rain_Region_ID_Work <- brm(bf(log(adjust.rc)~s(AH)+s(rain)+(1|region)+(1|idd)+work),
                                   data = analysis.tem.3,
                                   prior = prior,
                                   chains = 6,cores = 6,control = list(adapt_delta = 0.99),iter = 2000)
#save.image("mtrhrainsunshine_idd.RData")
save(B_AH_Rain_Region_ID_Work,file = paste(influenza_type,"_AH_Rain_Region_ID_Work_Region.RData",sep = "_"))
...
B_AH_Rain_Sun_Region_ID_Work <- brm(bf(log(adjust.rc)~s(AH)+s(rain)+s(sunshine)+(1|region)+(1|idd)+work),
                                       data = analysis.tem.3,
                                       prior = prior,
                                       chains = 6,cores = 6,control = list(adapt_delta = 0.99),iter = 2000)
#save.image("mtrhrainsunshine_idd.RData")
save(B_AH_Rain_Sun_Region_ID_Work,file = paste(influenza_type,"_AH_Rain_Sun_Region_ID_Work_Region.RData",sep = "_"))